...
userSchema.methods.updateAccountingBalances = function () {
  ...
  Object.keys(balances).forEach((code) => {
    balances[code].balance =
      balances[code].totalDebit - balances[code].totalCredit;
...
userSchema.methods.updateAssetStatistics = function () {
  ...
  this.assetStatistics.totalInitialValue = activeAssets.reduce(
    (sum, asset) => sum + asset.initialValue,
    0
  );
  this.assetStatistics.totalCurrentValue = activeAssets.reduce(
    (sum, asset) => sum + asset.currentValue,
    0
  );
  this.assetStatistics.totalAmortization = activeAssets.reduce(
    (sum, asset) => sum + asset.amortization,
    0
  );

  if (this.assetStatistics.totalInitialValue > 0) {
    this.assetStatistics.averageAmortizationPercentage =
      (this.assetStatistics.totalAmortization /
        this.assetStatistics.totalInitialValue) *
      100;
  }
...
userSchema.methods.updateDepartmentValues = function () {
  ...
  this.assets
    .filter((asset) => asset.status === "Aktiv")
    .forEach((asset) => {
...
userSchema.methods.calculateAmortization = function (asset) {
  ...
  const amortizationPercentage =
    asset.initialValue > 0 ? (amortization / asset.initialValue) * 100 : 0;
