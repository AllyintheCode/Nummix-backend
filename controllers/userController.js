  ...
  try {
    ...
    if (currency && !["AZN", "USD", "RUB", "EUR"].includes(currency)) {
      return res.status(400).json({
        success: false,
        message:
          'currency yalnız "AZN", "USD", "RUB", "EUR" dəyərlərindən biri ola bilər',
      });
    }
    ...
    if (
      status &&
      !["planned", "pending", "overdue", "completed"].includes(status)
    ) {
      return res.status(400).json({
        success: false,
        message:
          'status yalnız "planned", "pending", "overdue", "completed" dəyərlərindən biri ola bilər',
      });
    }
  ...
  try {
    ...
    if (!["planned", "pending", "overdue", "completed"].includes(status)) {
      return res.status(400).json({
        success: false,
        message:
          'Yalnız "planned", "pending", "overdue", "completed" statusları dəyişdirilə bilər',
      });
    }
    ...
    res.json({
      ...
      data: {
        ...
        breakdown: {
          ...
          tax_percentage: (
            (taxResult.totalEmployerTaxes / totalCost) *
            100
          ).toFixed(2),
        },
      },
  ...
  try {
    ...
    employees.forEach((employee) => {
      ...
      if (employeeType === "state") {
        employerTaxResult =
          taxCalculationService.calculateStateEmployerTaxes(employeeSalary);
      } else {
        employerTaxResult =
          taxCalculationService.calculatePrivateEmployerTaxes(employeeSalary);
      }
    ...
    const overallTaxResult =
      taxCalculationService.calculateEmployerTaxes(totalSalaryFund);
    ...
    user.company_taxes.total_company_taxes[month] =
      overallTaxResult.totalEmployerTaxes;
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          ...
          total_company_cost:
            totalSalaryFund + overallTaxResult.totalEmployerTaxes,
          average_salary_per_employee:
            employees.length > 0
              ? (totalSalaryFund / employees.length).toFixed(2)
              : 0,
          average_tax_per_employee:
            employees.length > 0
              ? (
                  overallTaxResult.totalEmployerTaxes / employees.length
                ).toFixed(2)
              : 0,
        },
        tax_breakdown: {
        ...
        cost_distribution: {
          salary_percentage:
            totalSalaryFund > 0
              ...
              : 0,
          tax_percentage:
            overallTaxResult.totalEmployerTaxes > 0
              ? (
                  (overallTaxResult.totalEmployerTaxes /
                    (totalSalaryFund + overallTaxResult.totalEmployerTaxes)) *
                  100
  ...
  try {
    ...
    const statistics = months.map((month) => {
      ...
      return {
        ...
        percentages: {
          salary:
            salaryFund > 0 ? ((salaryFund / totalCost) * 100).toFixed(2) : 0,
          ...
          total_tax:
            salaryFund > 0 ? ((totalTaxes / salaryFund) * 100).toFixed(2) : 0,
        },
      };
    });

    // İllik cəmlər
    const yearlySummary = statistics.reduce(
      (acc, monthStat) => {
    ...
    yearlySummary.averages = {
      dsmf_percentage:
        yearlySummary.total_salary_fund > 0
          ? (
              (yearlySummary.total_dsmf / yearlySummary.total_salary_fund) *
              100
            ).toFixed(2)
          : 0,
      ish_percentage:
        yearlySummary.total_salary_fund > 0
          ? (
              (yearlySummary.total_ish / yearlySummary.total_salary_fund) *
              100
            ).toFixed(2)
          : 0,
      its_percentage:
        yearlySummary.total_salary_fund > 0
          ? (
              (yearlySummary.total_its / yearlySummary.total_salary_fund) *
              100
            ).toFixed(2)
          : 0,
      total_tax_percentage:
        yearlySummary.total_salary_fund > 0
          ? (
              (yearlySummary.total_taxes / yearlySummary.total_salary_fund) *
              100
            ).toFixed(2)
          : 0,
      salary_cost_percentage:
        yearlySummary.total_cost > 0
          ? (
              (yearlySummary.total_salary_fund / yearlySummary.total_cost) *
              100
            ).toFixed(2)
          : 0,
    };
...
export const addEmployeeFlow = async (req, res) => {
  try {
    const { employeeId, type, date, department, position, reason, notes } =
      req.body;
  ...
  try {
    ...
    const stats = {
      ...
      byStatus: {
        ...
        "Sıradan çıxıb": assets.filter((a) => a.status === "Sıradan çıxıb")
          .length,
      },
    };
...
export const getAssetStatistics = async (req, res) => {
  try {
    ...
    const totalInitialValue = activeAssets.reduce(
      (sum, a) => sum + a.initialValue,
      0,
    );
    ...
    const totalAmortization = activeAssets.reduce(
      (sum, a) => sum + a.amortization,
      0,
    );
    ...
    res.json({
      ...
      data: {
        ...
        byStatus: Object.entries(statusStats).map(([status, count]) => ({
          ...
          count,
          percentage:
            assets.length > 0 ? ((count / assets.length) * 100).toFixed(2) : 0,
        })),
  ...
  try {
    ...
    res.json({
      ...
      summary: {
        ...
        totalValue,
        averageValue:
          assets.length > 0 ? (totalValue / assets.length).toFixed(2) : 0,
      },
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
...
export const getCategoryReports = async (req, res) => {
  try {
    ...
    const summary = {
      ...
      totalAssetsCovered: reports.reduce(
        (sum, r) => sum + r.summary.totalAssets,
        0,
      ),
      totalValueCovered: reports.reduce(
        (sum, r) => sum + r.summary.totalCurrentValue,
        0,
      ),
      averageAmortization:
        reports.length > 0
          ? reports.reduce(
              (sum, r) => sum + r.summary.averageAmortizationPercentage,
              0,
            ) / reports.length
          : 0,
    };
...
export const getCategoryReportById = async (req, res) => {
  try {
    ...
    const comparisonData = report.data.map((item) => {
      const current = currentCategoryMap.get(item.category) || {
        assetCount: 0,
        currentValue: 0,
      };
      return {
    ...
    res.json({
      ...
      data: {
        ...
        currentSummary: {
          ...
          totalCurrentValue: currentAssets.reduce(
            (sum, a) => sum + a.currentValue,
            0,
          ),
        },
      ...
      averageAmortizationPercentage:
        assets.length > 0
          ? (assets.reduce((sum, a) => sum + a.amortization, 0) /
              assets.reduce((sum, a) => sum + a.initialValue, 0)) *
            100
          : 0,
  ...
  try {
    ...
    const data = Array.from(locationMap.entries()).map(([location, stats]) => {
      const percentage =
        locationMap.size > 0
          ? (stats.currentValue /
              Array.from(locationMap.values()).reduce(
...
export const getDepartmentReports = async (req, res) => {
  try {
    ...
    const reports = await DepartmentReport.find(filter).sort({
      generatedAt: -1,
    });
...
export const getDepartmentReportById = async (req, res) => {
  try {
    ...
    const comparisonData = report.data.map((item) => {
      const current = currentLocationMap.get(item.location) || {
        assetCount: 0,
        currentValue: 0,
      };
      return {
...
export const generateRealTimeDepartmentReport = async (req, res) => {
  try {
    ...
    assets.forEach((asset) => {
      ...
      locationStats[asset.location].categories[asset.category].value +=
        asset.currentValue;
    ...
    const categoryData = Object.entries(categoryByLocation)
      .map(([category, stats]) => ({
        category,
        totalValue: stats.totalValue,
        locationDistribution: Object.entries(stats.locations)
          .map(([location, count]) => ({
            location,
            count,
            percentage:
              locationTotals.totalAssets > 0
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          ...
          averageValuePerLocation:
            Object.keys(locationStats).length > 0
              ? (
                  locationTotals.totalValue / Object.keys(locationStats).length
                ).toFixed(2)
              : 0,
          averageAssetsPerLocation:
            Object.keys(locationStats).length > 0
              ? (
                  locationTotals.totalAssets / Object.keys(locationStats).length
                ).toFixed(1)
              : 0,
        },
        generatedAt: new Date(),
...
export const updateCompanyTaxes = async (req, res) => {
  try {
    ...
    if (!user.company_taxes.total_company_taxes)
      user.company_taxes.total_company_taxes = {};
      ...
      summary: {
        ...
        payment_success_rate:
          employeePayments.length > 0
            ? ((completedPayments / employeePayments.length) * 100).toFixed(2)
            : 0,
      },
...
const updateUserPaymentStats = async (userId, payment) => {
  ...
  const month = new Date(payment.forMonth).toLocaleString("en-US", {
    month: "long",
  });
...
const updateUserEmployeeFlowStats = async (userId, flow) => {
  ...
  if (user.monthly_employee_flow && user.monthly_employee_flow[month]) {
    if (flow.type === "hired") {
      ...
      const field =
        flow.type === "terminated" ? "terminations" : "resignations";
